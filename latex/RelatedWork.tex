...
$hat{V}(\mathrm{x}, \mathrm{x}_i)$ is the visibility of a virtual light and $\phi_i$ its incoming flux.
...
Also, the technique handles only a single indirect bounce of diffuse lighting since the VPLs are strictly speaking hemispherical Lambert emitter.
...
Note that the geometry term is a combination of the photometric distance law and the definition of intensity for Lambert emitter as described earlier in \autoref{sec:theo:relation}. % since VPLs are dimension-less Lambert emitters.
\\
\begin{figure}[h]
\centering
...
\begin{align}
G(\mathrm{x}, \mathrm{x}_i)_{VAL} = \frac{(\hat{\mathbf{n}} \cdot \overrightarrow{\mathrm{x}\mathrm{x}_i} )^+}{||\mathrm{x} - \mathrm{x}_i||^2 + A_{i}} \cdot (\hat{\mathbf{n}}_i \cdot \overrightarrow{\mathrm{x}_i\mathrm{x}})^+
\end{align}
...
To be able to evaluate quickly arbitrary specular exponents (the paper makes use of the Phong BRDF), the result is pre-convolved using mipmapping.
...
Additionally it uses cone tracing (which will be explained in \autoref{sec:prev:voxelmethods}) in a prefiltered voxel grid instead of rasterization to apply the indirect radiance.
...
Like LPV it is able to capture low frequency diffuse lighting but has less limitations in light transport distance.
