...
All three scenes have very different detail densities and feature different mixtures of materials.
...
Its intensely contrasted curtain colors are a popular show case for global illumination effects.
...
The strong changes in the duration of the G-buffer creation pass (especially in \dataset{Sponza}) stem from varying overdraw depending on the current view orientation.
...
Since we are only interested in the effects of the RSM resolution we canceled that factor out by increasing the shadow lod so that the number of cones stays at 256.
\\
...
The brightening originates primarily from the thin voxelization which yields too low blocking values in higher voxel volume mipmaps.
...
\end{subfigure}
\caption{Indirect shadow for different shadow lods in the \dataset{Sponza} scene using a voxel volume resolution of $256^3$ and CAV resolution of $64^3$. High lods suffer from the low density in higher voxel mipmaps which is the result of thin voxelization (as opposed to solid voxelization). }
...
\hspace{0.5cm} \emph{Depth}  & $1024^2 \cdot \SI{4}{\byte}$ & \textit{4\,MiB}\\ %\SI{4}{\mebi\byte} \\
\midrule
...
Techniques with even fewer pre-computations like LightSkin can alleviate this problem, but may still suffer from missing detail for smaller features.
\\
