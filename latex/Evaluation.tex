...
\end{figure}
\autoref{fig:groundtruthcomparision} shows a comparison between the results of a pathtracer and our technique of a simple diffuse-only scene.
...
Due to the limited view range we chose used a single adress volume with a resolution of $128^3$, other than that we use the default settings but without specular lighting since we were not able to replicate our BRDF setup in the Mitsuba \cite{bib:mitsuba} path-tracer.
With this high cache frequency our approach is able to hint most shadowing details however fails to compute them accurately.
...
\todo{concluding notes on how well it performs against pathtracing..?}

\subsection{SH Band Comparision}
\autoref{fig:shcomparision}
\todo{write on SH01 vs SH012}
\begin{figure}[h]
\centering
\begin{subfigure}[b]{0.8\textwidth}
