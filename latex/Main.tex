...
\\
While it can be very beneficial to place caches freely, only depending on view space information this also may introduce several temporal artifacts if caches disappear or move. %, i.e. strong changes in indirect lighting from one frame to the next.
...
\end{figure}
Similar to Cascaded Light Propagation Volumes \cite{bib:lpt} a set of nested CAV cascades are centered on the camera.
...
We found that the method has some weak-points that will be later discussed in \autoref{}. \todo{section ref needed}
\begin{figure}[h!]
\centering
...
\todo{review if non dynamic lod is clear}
...
The mip-level is computed similar as $\gamma_{max}$ before, only that this time the mip-level $m$ is searched for.
\begin{align}
...
To parameterize the Blinn Phong BRDF, all surfaces have a reflectance texture (sRGB), a tangent space normal-maps and two material property textures which are combined into a single texture during loading if not already in desired format.
...
\autoref{TODO} \todo{ref eva} discusses which technique performs better.
