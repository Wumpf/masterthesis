...
Since only a small fraction of all grid cells contains any caches, they are stored in a separate buffer and only their addresses within this buffer are saved in the grid.
...
If however an affected cell already contains a valid cache address, it is skipped as pixels do not introduce any data besides the cache allocation itself in this pass.
...
\begin{figure}[h!]
\centering
\subcaptionbox{Cache visualization displaying Specular Environement Map.}{
...
Using this metallic value, the reflectance is split up into diffuse and specular reflectance using simple heuristics.
\\
Additionally, our rendering system samples a normalmap for all surfaces.
...
\\
Our final version works with shared memory and a simple heuristic that does not rely on shared memory:
\begin{lstlisting}[language=GLSL]
...
\end{figure}
Our shadow LOD algorithm requires that a shadowing value is valid for multiple virtual lights.
